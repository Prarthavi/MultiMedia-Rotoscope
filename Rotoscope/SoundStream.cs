...
namespace Rotoscope
{
    class SoundStream : Sound
    {
        ...
        private AudioFileReader readerStream;
        ...
        override public float[] GetFrame(int index)
        {
            ...
            readerStream.Read(data, 0, 8);
            float[] temp = ByteToFloat(data);
        ...
        {
            //check for support and preprocess
            if (!base.Open(path))
                return false;
        ...
        private bool OpenFileStream(string path)
        {
            ...
            {
                //open file
                readerStream = new AudioFileReader(path);
