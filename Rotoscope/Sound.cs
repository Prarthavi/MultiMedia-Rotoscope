...
namespace Rotoscope
{
    ...
    public abstract class Sound : WaveProvider32
    {
        ...
        public virtual bool isSupportedForOpen(SoundFileTypes index)
        {
            try
            {
        ...
        public SoundFileTypes pathToFileType(string path)
        {
            FileInfo fi = new FileInfo(path);
            string ext = fi.Extension.Remove(0, 1).ToUpper();
            SoundFileTypes type;
            if (Enum.TryParse(ext, out type))
                return type;
            else
                return SoundFileTypes.UNSUPPORTED;
        ...
        {
            FileInfo fi = new FileInfo(path);
            SoundFileTypes type = pathToFileType(path);

            if (!isSupportedForOpen(type))
            {
                return false;
            }
            else
